...
description: 生成「AI影响力信息汇总」周报：在不使用 X API 的前提下，用 opencli 的只读 Google/X 搜索批量扫描指定账号过去7天推文（偏工具/工作流/教程/Prompt），过滤出对内容创作者立刻可用的高价值内容，产出结构化中文周报 Markdown，并通过邮件发送给订阅者。当你需要做每周 AI Builder 账号扫描、实用推文精选、工作流/方法论周报时使用。
---
...
- 允许：已登录浏览器会话复用的 `opencli google search` / `opencli twitter search`（只读）+ X 公共 syndication timeline + X 官方 `oEmbed` + 本地整理与邮件发送。
...
将步骤 2 生成的 `weekly_report.md` 通过邮件发送给订阅者。

**前置检查：**
必须使用 send-email skill 发送邮件，如果没找到 send-email skill，输出没找到的原因，然后终止流程

**发送逻辑：**

- 目标收件人：`chenliang535649@163.com`、`nipuream@163.com`
- 主题：格式为 `YYYYMMDD_X平台高质量内容`，例如 `20260503_X平台高质量内容`，日期取周报对应的日期
- 正文：用 --file 的方式带上 `weekly_report.md` 的绝对路径
- 其他参数
...
**发送后验证：**
- 检查脚本退出码为 0
- 如发送失败，检查授权码是否正确、网络是否可达 `smtp.163.com:465`
...
- `references/accounts.txt`：默认 X 账号清单
